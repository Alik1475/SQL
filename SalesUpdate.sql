...
BEGIN
	...
	if OBJECT_ID('tempDB_UAT..#t', 'U') is not null drop table #t

	if OBJECT_ID('tempDB_UAT..#t1', 'U') is not null drop table #t1

	if OBJECT_ID('tempDB_UAT..#t3', 'U') is not null drop table #t3

	if OBJECT_ID('tempDB_UAT..#t4', 'U') is not null drop table #t4
...
select ss.user_id fatherID, ss.account_id ChildID, up.UserCode ownerfirmid, s1.SubAccCode SubAccCodeAN, ss.id idR
...
select sa.SubAccCode SubAccCodeSales, sa.id IDsubacc, frS.name, frS.id IDSales
...
select *,t1.SubAccCodeSales SubAccCodeSales1

into #t3

from #t t

left join #t1 t1 on t.OwnerFirmID = t1.IDSales and t.childID = t1.IDsubacc
