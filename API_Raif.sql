...
ALTER TABLE #ParsedResults
			...
			WITH RowNumberCTE AS (
				...
				IntradayOperationId,
					...
					ROW_NUMBER() OVER (ORDER BY IntradayOperationId) AS RowNum
	...
	insert into QORT_BACK_TDB.dbo.CorrectPositions( 
		...
		while (@WaitCount > 0 and exists (select top 1 1 from QORT_BACK_TDB.dbo.CorrectPositions q with (nolock) where q.IsProcessed in (1,2)))
...
left outer join QORT_BACK_DB..CorrectPositions corr on corr.BackID = pars.IntradayOperationId and corr.IsCanceled = 'n'
