...
exec QORT_ARM_SUPPORT.dbo.exportTrades_RegulatoryNY11FX_2 '20240115'
...
CREATE PROCEDURE [dbo].[exportTrades_RegulatoryNY11FX_2]
...
BEGIN
	...
	begin try
		...
		--declare @TemplateFileName varchar(255) = '\\192.168.14.22\Exchange\QORT_Files\Regulatory Reports\NY11_workTemplate (FX)\template\42000_NY11_workTemplate_14a_test.xls'
...
ar, getdate(), 108), ':', '') + '.xls'
		...
		declare @ResultFileName varchar(255) = '\\192.168.14.22\Exchange\QORT_Files\Regulatory Reports\NY11_workTemplate (FX)\test_42000_NY11_'+cast(@TradeDateTo as varchar)+'.xls'
		...
		--if OBJECT_ID('tempdb..##42000_NY11_workTemplate_V2', 'U') is not null drop table ##42000_NY11_workTemplate_V2
...
--			and NOT (t.TradeDate = @TradeDateTo and t.TradeTime >= @TradeTimeFrom)
		...
		inner join (
			...
			select r.CurrencyCell, r.IsFirstCell, count(*) tc, sum(r.TradeQty * r.TradeRate) / sum (r.TradeQty) AvgWeightRate, sum(1 * r.TradeRate) / sum (1) AvgRate
		...
		/*outer apply(select cast(r.TradeQty as float) Qty

				, cast(r.AvgWeightRate as float) Rate1

				, cast(r.AvgRate as float) Rate2
...
*/
		...
		SET @sql = 'insert into OPENROWSET (
			...
			''Excel 5.0; Database='+ @TempFileName + ';HDR=YES;IMEX=3'',
		...
		SET @sql = 'select * into ##42000_NY11_workTemplate_V2 from OPENROWSET (

			''Microsoft.ACE.OLEDB.12.0'',

			''Excel 5.0; Database='+ @TemplateFileName2 + ';HDR=YES;IMEX=3'',
		...
		from ##42000_NY11_workTemplate_V2
		...
		SET @sql = 'insert into OPENROWSET (
			...
			''SELECT * FROM [' + @Sheet + '$A10:K11]'')

			select top 1 * from ##42000_NY11_workTemplate_V2'
