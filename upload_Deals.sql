...
BEGIN
	...
	begin try
		...
		select @TradeNumStart = isnull(max(tradeNum), @TradeNumStart) from QORT_BACK_DB.dbo.Trades with (nolock) where TradeNum >= @TradeNumStart
		...
		declare @FileName varchar(128) = '\\192.168.14.22\Exchange\QORT_Files\PRODUCTION\Deals.xlsx'
		...
		FROM OPENROWSET (
			...
			''SELECT * FROM [' + @Sheet + '$A1:ZZ1000000]'')'
		...
		-- select top 100 aid, IsProcessed, ErrorLog, * from QORT_BACK_TDB.dbo.ImportTrades it with (nolock)

		-- select top 100 * from QORT_ARM_SUPPORT.dbo.uploadLogs with (nolock) order by 1 desc



		set @aid = isnull((select max(aid) from QORT_BACK_TDB.dbo.ImportTrades with (nolock)), 0)
		...
		SELECT 1 as IsProcessed, 2 as ET_Const, 'y' as IsDraft 
			...
			, tt.BuySell, d.Instrument SecCode, cast(d.Amount as float) Qty, round(cast(d.Price as float), 8, 0) Price

			, case when a.AssetType_Const = 3 then round(case when a.AssetClass_Const in (6,7,9) then (cast(d.Amount as float)*cast(d.Price as float)*cast(isnull(d.CrossRate, 1) as float) * a.BaseValueOrigin/100) + case when tt.AccInVolume = 'y' then cast(dACI * c
ast(isnull(d.CrossRate,1) as float) as float) else 0 end else cast(d.Amount as float) * cast(isnull(d.CrossRate,1) as float)*cast(d.Price as float) end, 2, 0)

				else round(case when a.AssetClass_Const in (6,7,9) then (cast(d.Amount as float)*cast(d.Price as float)*cast(isnull(d.CrossRate,1) as float)*a.BaseValueOrigin/100) + case when tt.AccInVolume = 'y' then cast(dACI*cast(isnull(d.CrossRate,1) as float) as
 float) else 0 end else cast(d.Amount as float) * cast(isnull(d.CrossRate,1) as float)*cast(d.Price as float) end, 2, 0)

				end Volume1

			, case when a.AssetType_Const = 3 then round(case when a.AssetClass_Const in (6,7,9) then (cast(d.Amount as float)*cast(d.Price as float)*cast(isnull(d.CrossRate,1) as float)*a.BaseValueOrigin/100) + case when tt.AccInVolume = 'y' then cast(dACI*cast(i
snull(d.CrossRate,1) as float) as float) else 0 end end, 2, 0)

				else round(case when a.AssetClass_Const in (6,7,9) then (cast(d.Amount as float)*cast(d.Price as float)*cast(isnull(d.CrossRate,1) as float)*a.BaseValueOrigin/100) + case when tt.AccInVolume = 'y' then cast(dACI*cast(isnull(d.CrossRate,1) as float) as
 float) else 0 end end, 2, 0)
			...
			, cast(
				...
				(cast(
						...
						right(d.[Sub-Account],4) as varchar(8))+cast(isnull((select max(ID) from QORT_BACK_DB.dbo.Trades with (nolock))+1,0) as varchar(8))) as int) TradeNum
		...
		outer apply (select top 1 uTrader.id from QORT_BACK_DB.dbo.Users uTrader with (nolock) where uTrader.last_name + ' ' + uTrader.first_name = d.[Execution Trader] collate Cyrillic_General_CS_AS and uTrader.Enabled = 0 order by 1 desc) uTrader

		outer apply (select top 1 uSales.id from QORT_BACK_DB.dbo.Users uSales with (nolock) where uSales.last_name + ' ' + uSales.first_name = d.[Investment Decision] collate Cyrillic_General_CS_AS and uSales.Enabled = 0 order by 1 desc) uSales

		left outer join QORT_BACK_DB.dbo.TSSections tss with (nolock) on tss.Name = d.Section and tss.Enabled = 0

		left join QORT_BACK_DB.dbo.Trades t with (nolock) on t.TradeDate = cast(convert(varchar, d.Date, 112) as int) and t.BuySell = tt.BuySell and t.AgreeNum = cast(d.AgreeMent as varchar) and t.Enabled = 0 and t.nullstatus = 'n'

		--outer apply (select top 1 * from QORT_BACK_DB.dbo.Trades t with (nolock) where t.id = tp.Trade_ID and t.TradeDate = cast(convert(varchar, d.Date, 112) as int) and t.BuySell = tt.BuySell /*and t.AgreeNum = cast(d.AgreeMent as varchar)*/ and t.Enabled =
 0 and t.nullstatus = 'n') t

		/*outer apply (

			select top 1 t.* 

			from QORT_BACK_DB.dbo.TradeProperties tp with (nolock) 

			inner join QORT_BACK_DB.dbo.Trades t with (nolock) on t.id = tp.Trade_ID and t.TradeDate = cast(convert(varchar, d.Date, 112) as int) and t.BuySell = tt.BuySell /*and t.AgreeNum = cast(d.AgreeMent as varchar)*/ and t.Enabled = 0 and t.nullstatus = 'n'
		...
		left join QORT_BACK_DB.dbo.Securities sec with (nolock) on sec.SecCode = d.Instrument and sec.TSSection_ID = tss.id

		left join QORT_BACK_DB.dbo.Assets a with (nolock) on a.id = sec.Asset_ID

		left join QORT_BACK_DB.dbo.Accounts aPut with (nolock) on aPut.AccountCode = d.DeliveryAccount collate Cyrillic_General_CS_AS

		left join QORT_BACK_DB.dbo.Accounts aPay with (nolock) on aPay.AccountCode = d.[Payment Account] collate Cyrillic_General_CS_AS
...
--return
		...
		set @rows = @@ROWCOUNT; if @rows > 0 begin set @Message = 'File Uploaded - "'+@filename+'": ' + cast(@rows as varchar) + ' new deals'; insert into QORT_ARM_SUPPORT.dbo.uploadLogs(logMessage, errorLevel, logRecords) values (@message, 2001, @rows); end;
		...
		insert into QORT_BACK_TDB.dbo.ImportTrades (ET_Const, IsProcessed, SystemID, AgreeNum)
		...
		inner join QORT_BACK_DB.dbo.Trades t with (nolock) on t.TradeDate = cast(convert(varchar, d.Date, 112) as int) and t.BuySell = tt.BuySell and t.AgreeNum = cast(d.AgreeMent as varchar) and t.Enabled = 0 and t.nullstatus = 'n'

		inner join QORT_BACK_DB.dbo.Trades t2 with (nolock) on t2.id = t.CpTrade_ID and t2.AgreeNum = cast(d.Agreement as varchar) and t2.Enabled = 0 and t2.nullstatus = 'n'
		...
		set @rows = @@ROWCOUNT; if @rows > 0 begin set @Message = 'File Uploaded - "'+@filename+'": ' + cast(@rows as varchar) + ' AgreeNum2 modified'; insert into QORT_ARM_SUPPORT.dbo.uploadLogs(logMessage, errorLevel, logRecords) values (@message, 2001, @rows
); end;
		...
		from QORT_BACK_TDB.dbo.ImportTrades a with (nolock)
		...
		--inner join QORT_BACK_DB.dbo.TradeProperties tp with (nolock) on tp.ExternalNum = cast(d.AgreeMent as varchar)

		--inner join QORT_BACK_DB.dbo.Trades t with (nolock) on t.id = tp.Trade_ID and t.TradeDate = cast(convert(varchar, d.Date, 112) as int) and t.BuySell = tt.BuySell and t.Enabled = 0 and t.nullstatus = 'n'
		...
		left outer join QORT_BACK_DB.dbo.Subaccs gs with (nolock) on gs.SubAccCode = cast(t.SubAccForCrediting as varchar(128)) collate Cyrillic_General_CS_AS
