...
AR(64) = 'Sheet1';
        ...
        DECLARE @NotifyEmail VARCHAR(1024) = 'milena.ghayfajyan@armbrok.am;maxim.biryukov@armbrok.am;tigran.gevorgyan@armbrok.am;backoffice@armbrok.am;QORT@armbrok.am'

        IF OBJECT_ID('tempdb.
.#t', 'U') IS NOT NULL DROP TABLE #t
        ...
        LEFT OUT
ER JOIN QORT_BACK_DB.dbo.Assets f1 ON f1.id = ass.BaseCurrencyAsset_ID
        WHERE CP.EndDate = @todayInt
        
        SELECT * FROM #t 

        -- Блок формирования части уведомления о сделках в процессе
        IF OBJECT_ID('tempdb..#t1', 'U') IS
 NOT NULL DROP TABLE #t1
        ...
        SELECT tr.id
             ...
             , CASE 
                ...
                WHEN (tr.RepoTrade_ID > 0 AND Tr.BuySell = 1 AND tr.IsRepo2 = 'n') 
                   ...
                   WHEN (tr.RepoTrade_ID < 0 AND Tr.BuySell = 1 AND tr.IsRepo2 = 'n') 
                        OR (tr.IsRepo2 = 'y' AND Tr.BuySell = 2) THEN 'Buy'
...
lume1
             ...
             , f1.Name AS CurCoupon
        INTO #t1

        FROM QORT_BACK_DB.dbo.Coupons CP
        INNER JOIN QORT_BACK_DB.dbo.Assets ass ON ass.id = CP.Asset_ID
        LEFT OUTER JOIN QORT_BACK_DB.dbo.Firms f ON f.id = ass.EmitentFirm_ID
        LEFT OUTER JOIN QORT_BACK_DB.dbo.Assets f1 ON f1.id = ass
.BaseCurrencyAsset_ID
        ...
        LEFT OUTER JOIN QORT_BACK_DB.dbo.Firms fcp ON fcp.ID = Tr.CpF
irm_ID
...
RCHAR(MAX))
                ...
                SET @NotifyMessage1 = REPLACE(@NotifyMessage1, '//3\\', '
</td></tr>')
                ...
                SET @NotifyMessage1 = '<br/><br/><b>'
                    + '<b
r><br>Trade in process now: '
                    + '<table border="1"><tr BGColor="#CCCCCC"><font color="black"/>'
                    + '<td>Trade_ID</td><td>Counterparty</td><td>Instrument</td><td>EmitentAsset</td><td>ISIN</td><td>TradeType</td><td>Qty
</td><td>Volume</td><td>RepoRate</td><td>RecordDate</td><td>EventType</td><td>Payment amount</td></tr>'
        ...
        + '<br><br>Trade in process now: '
            ...
            SET @NotifyMessage = @NotifyMessage + @NotifyMessage1
            S
ET @NotifyTitle = 'Alert!!! Assets with redemtion today'
