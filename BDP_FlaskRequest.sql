...
-- exec QORT_ARM_SUPPORT..BDP_FlaskRequest @IP= '192.168.13.80', @IsinCode = 'USF1067PAF39 corp'
...
BEGIN
    ...
    IF OBJECT_ID(
'tempdb..#Curloutput_TEST') IS NOT NULL DROP TABLE #Curloutput_TEST;
    ...
    IF OBJECT_ID('tempdb..#ParsedResults') IS NOT NULL DROP TABLE #ParsedResults;
	    ...
	    DECLARE @Message VARCH
AR(1024) -- для уведомлений об ошибках
        DECLARE @todayDate DATE = GETDATE()
        DECLARE @todayInt INT = CAST(CONVERT(VARCHAR, @todayDate, 112) AS INT)
		DECLARE @Fields NVARCHAR(MAX) = '\"NAME\",\"PX_CLOSE_1D\",\"CPN\",\"DS027\",\"INDUSTRY_SECT
OR\",\"YAS_BOND_YLD\",\"AMT_OUTSTANDING\",\"DX129\",\"SECTORAL_SANCTIONED_SECURITY\",\"OFAC_SANCTIONED_SECURITY\",\"EU_SANCTIONED_SECURITY\",\"UK_SANCTIONED_SECURITY\",\"DX657\",\"LONG_COMPANY_NAME_REALTIME\",\"ISSUER_BULK\",\"SECURITY_NAME\",\"DS122\",\"
DS674\",\"EQY_PRIM_SECURITY_CRNCY\",\"CRNCY\",\"PAR_AMT\",\"AMT_ISSUED\",\"ISSUE_DT\",\"MATURITY\",\"DS497\",\"CPN_FREQ\",\"NXT_CPN_DT\",\"INT_ACC\"';
	...
	declare @Curloutput TABLE (output NVARCHAR(M
AX));
...
urrentRow;
        ...
        SET @CurlCommand = 'curl -X POST http://' + @IP + ':5001/get_data -H "Content-Type: application/json" -d "{\"isin_codes\": [' + @IsinCode + '], \"field
s\": ['+ @Fields +']}"';
		print @CurlCommand
        BEGIN TRY
            ...
            SELE
CT @JsonResult = STRING_AGG(CAST(output AS NVARCHAR(MAX)), '')
            FROM #Curloutput_TEST
            WHERE output IS NOT NULL;

            -- Очистка результата от всего перед первым символом '['
            SET @JsonResult = SUBSTRING(@JsonResul
t, CHARINDEX('[', @JsonResult), LEN(@JsonResult));
...
d';
      ...
      Sectoral_Sanctioned_Security, OFAC_Sanctioned_Security, EU_SANCTIONED_SECURITY, UK_Sanctioned_Security, DX657, 
                    ...
                    C
rncy, Par_Amt, Amt_Issued, Issue_Dt, Maturity, DS497, Cpn_Freq, Nxt_Cpn_Dt, Int_Acc, Found, ErrorMessage)
                SELECT 
					@todayInt as date,
...
Data.value, '$.NAME') AS Name,
                ...
                TRY_CONVERT(VARCHAR(8), DATEADD(MINUTE, CAST(JSON_VALUE(jsonData
.value, '$.DS027') AS bigint)/60000, '1970-01-01'), 112)    /*JSON_VALUE(jsonData.value, '$.DS027') */AS DS027,			
                    JSON_VALUE(jsonData.value, '$.INDUSTRY_SECTOR') AS Industry_Sector,
                    JSON_VALUE(jsonData.value, '$.YA
S_BOND_YLD') AS YAS_BOND_YLD,
                    JSON_VALUE(jsonData.value, '$.AMT_OUTSTANDING') AS Amt_Outstanding,
                    JSON_VALUE(jsonData.value, '$.DX129') AS DX129,
                    JSON_VALUE(jsonData.value, '$.SECTORAL_SANCTIONED
_SECURITY') AS Sectoral_Sanctioned_Security,
   ...
   JSON_VALUE(jsonData.value, '$.UK_SANCTIONED_SECURITY') AS UK_Sanctioned_Security,
                    ...
                    JSON_VALUE(jsonData.value, '$.LONG_COMPANY_NAME_REALTIME') AS Long_Company_Name_Re
altime,
     ...
     JSON_VALUE(jsonData.value, '$.DS674') AS DS674,
                    ...
                    JSON_VALUE
(jsonData.value, '$.PAR_AMT') AS Par_Amt,
...
AS Maturity,
                    ...
                    JS
ON_VALUE(jsonData.value, '$.INT_ACC') AS Int_Acc,
