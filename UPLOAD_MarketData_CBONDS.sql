...
-- exec QORT_ARM_SUPPORT..UPLOAD_MarketData_CBONDS

CREATE PROCEDURE [dbo].[UPLOAD_MarketData_CBONDS]
...
BEGIN
		...
		declare @FilePath varchar(255) = '\\192.168.14.22\Exchange\QORT_Files\MarketDataCBONDS\'
	...
	begin
		...
		, (case 
					when ass.assetClass_Const in (6) then iif(t.bond = 0, t.bondDubl, t.bond)
					when ass.assetClass_Const not in (6) then t.Equity 				
					else 0 
				 end) as MarketPrice2
		...
		, iif(ass.assetClass_Const not in (6), t.CurencyEQ, null) as PriceAsset_ShortName

		, (cast(convert(varchar, t.Date , 112) as int))  SettleDate
