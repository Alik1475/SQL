...
S SanctionQ
        ...
        INTO ##result
        FROM #t t
        FULL OUTER JOIN QORT_BACK_DB.dbo.Assets q ON t.ISIN = q.I
SIN and q.Enabled <> q.id
		...
		LEFT OUTER JOIN QO
RT_BACK_DB.dbo.Assets f WITH (NOLOCK) ON f.id = s.CurrPriceAsset_ID and f.id <> f.Enabled
		LEFT OUTER JOIN QORT_BACK_DB.dbo.Coupons cou WITH (NOLOCK) ON cou.Asset_ID = q.ID and cou.id <> cou.Enabled and cou.IsCanceled = 'n' and cou.BeginDate <= @todayInt
...
1 couL.EndDate as EndDate from QORT_BACK_DB.dbo.Coupons couL 
  ...
  SELECT CASE
     ...
     + IIF(t.Sanction <> q.IsInSanctionList, ', Sanction!!!', '')
                    ...
                    + IIF(t.Issue_date <> q.EmitDate AND t.Issue_date <
> 0, ', Issuer_Date', '')
			...
			+ IIF((t.crncy <> isnull(f.name,'')and (q.AssetClass_Const not IN(6,7,9))) , ', Currency:'+t.crncy, '')
					+ IIF((cast(isnull(t.cpn , isnull(cou.Procent,0)) as float) <> iif(isnull(cou.Procent,0) = 0, iif(CHARINDEX('/', cou.Description) >0 ,cast(SUBS
TRING(cou.Description, CHARINDEX('/', cou.Description, CHARINDEX('/', cou.Description) + 1) + 1, LEN(cou.Description)) as float),0), isnull(cou.Procent,0)) and (q.AssetClass_Const IN(6,7,9))) , ', Coupon%:'+cast(t.cpn AS varchar(16))+'_Q:'+cast(isnull(cou
.Procent,0) as varchar(16)), '')
					+ IIF(
   ...
   SELECT CASE
        ...
        WHER
E (q.Enabled <> q.id OR q.Enabled IS NULL) AND (q.AssetType_Const = 1 OR q.AssetType_Const IS NULL) AND q.Marking <> 'XS1207654853';

        SELECT * FROM ##result ORDER BY ISINQ;
		--return
        -- Отправка email с отчетом
        IF EXISTS (SELECT I
SIN FROM ##result WHERE Result <> 'OK')
        BEGIN
            ...
            DECLARE @Sh
eetClient VARCHAR(32) = 'Assets';
            DECLARE @fileTemplate VARCHAR(512) = 'template_Asset_Check_Bloomberg.xlsx';
            DECLARE @fileReport VARCHAR(512) = 'Asset_Check_Bloomberg_' + CONVERT(VARCHAR, GETDATE(), 112) + '_' + REPLACE(CONVERT(VA
RCHAR, GETDATE(), 108), ':', '-') + '.xlsx';
            DECLARE @cmd VARCHAR(512);
            DECLARE @sql2 VARCHAR(1024);

            -- Копирование шаблона отчета
            SET @cmd = 'copy "' + @FilePath + @fileTemplate + '" "' + @FilePath + @file
Report + '"';
        ...
        DECLARE @NotifyTitle VARCHAR(1024) = NULL;
            SET @NotifyMessage = CAST((
                SELECT '//1\\' + IIF(tt.ISIN IS NULL, 'NOT FOUND!!!', CAST(tt.ISIN AS VARCHAR))
                    + '//2\\' + ISNULL(CAST(tt.ISINQ AS VARCHAR), 'N
OT FOUND!!!')
...
AX));
    ...
    SET @NotifyMessage = REPLACE(@NotifyMessage, '//4\\', '</td><td ');
            ...
            SET @NotifyMessage = '<br><br><table border="1"><tr BGColor="#CCCCCC"><font color="black"/>'
                           ...
                           + '<td>ISIN(Bloomberg)</td><td>ISIN(Qort)</td><td>IssuerDate(Bloomberg)</td><td>IssuerDate(Qort)</td><td>Ticker(Bloomberg)</td><td>AssetShortName(Qort)</td><td>Nominal(Bloomberg)</td><td>Nominal(Qort)</td><td>MaturityDate(Bloomb
erg)</td><td>MaturityDate(Qort)</td><td>Issuer(Bloomberg)</td><td>Issuer(Qort)</td><td>Sanction</td><td>SanctionQ</td><td>Result</td></tr>'
                                + @NotifyMessage + '</table>';

            SET @fileReport = @FilePath + @fileRepo
rt;
