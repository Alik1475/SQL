...
--exec QORT_ARM_SUPPORT.dbo.upload_CrossRates_CBA



/*

	--insert into QORT_BACK_TDB.dbo.CrossRates(IsProcessed, Date, TradeAsset_ShortName, Bid, Ask, PriceAsset_ShortName, InfoSource, Qty)
	...
	set @sql = cast((
		...
		select ' union all select '''+s.name+''' TableName, IsProcessed, count(*) tc from QORT_BACK_TDB.dbo.' + s.name + ' with (nolock) where IsProcessed in (1,2) group by isProcessed'

		from QORT_BACK_TDB.sys.columns c with (nolock)

		inner join QORT_BACK_TDB.sys.objects s with (nolock) on s.object_id = c.object_id
...
--/*
		...
		from QORT_ARM_SUPPORT.dbo.uploadFileLocations l with (nolock)
		...
		if @locationId is null begin
			...
			insert into QORT_ARM_SUPPORT.dbo.uploadFileLocations(locationPath, locatoinType)
		...
		inner join QORT_ARM_SUPPORT.dbo.uploadFileNames uf with (nolock) on uf.locationId = @locationId and uf.fileName = f.fname
		...
		while 2 > 1 begin
			...
			from (
				...
				from QORT_ARM_SUPPORT.dbo.fnt_ParseString_Num((select top 1 cols from @t), @RowDelim)
			...
			outer apply (
				...
				select * from QORT_ARM_SUPPORT.dbo.fnt_ParseString_Num(t.val, ',')
			...
			set @aid = isnull((select max(aid) from QORT_BACK_TDB.dbo.CrossRates with (nolock)), 0)
			...
			set @rows = @@ROWCOUNT; if @rows > 0 begin set @Message = 'File Uploaded - "'+@filename+'": ' + cast(@rows as varchar) + ' rates'; insert into QORT_ARM_SUPPORT.dbo.uploadLogs(logMessage, errorLevel, logRecords) values (@message, 2001, @rows); end;



			set @WaitCount = 1200

			while (@WaitCount > 0 and exists (select top 1 1 from QORT_BACK_TDB.dbo.CrossRates t with (nolock) where t.IsProcessed in (1,2)))
			...
			from QORT_BACK_TDB.dbo.CrossRates t with (nolock)

			where t.aid > @aid and t.IsProcessed = 4



			insert into QORT_ARM_SUPPORT.dbo.uploadFileNames(locationId, fileName)
		...
		--- добавление в текущую дату курсов за предыдущий день СВА-1---- для калькулятора начисления брокерских комиссий-----------------

		insert into QORT_BACK_TDB.dbo.CrossRates(IsProcessed, Date, TradeAsset_ShortName, Bid, Ask, PriceAsset_ShortName, InfoSource, Qty)

		select 1 IsProcessed, cast (replace (dateadd(day, 1, cast(cast (c.OldDate as varchar) as date)), '-', '') as INT) Date, a.ShortName TradeAsset, c.Bid Bid, c.Ask Ask, b.ShortName PriceAsset, 'CBA-1' InfoSource, 1 Qty

		from QORT_BACK_DB.dbo.CrossRatesHist c

		left outer join QORT_BACK_DB.dbo.Assets a on a.id = c.TradeAsset_ID

		outer apply (select ShortName from  QORT_BACK_DB.dbo.Assets b where b.id = c.PriceAsset_ID) b

		where c.InfoSource = 'CBA' and c.OldDate = cast (replace (dateadd(day, -1, cast(cast (GETDATE() as varchar) as date)), '-', '') as INT)
...
--*/
		...
		SET @CURUSD = (SELECT BID FROM QORT_BACK_DB.dbo.CrossRates WHERE  PriceAsset_ID = 17 AND TradeAsset_ID = 8 and InfoSource = 'CBA')

		insert into QORT_BACK_TDB.dbo.CrossRates(IsProcessed, Date, TradeAsset_ShortName, Bid, Ask, PriceAsset_ShortName, InfoSource, Qty)

		select 1 IsProcessed

		, @todayInt Date

		, iif(a.ShortName = 'USD', 'AMD', a.ShortName) TradeAsset

		, iif(a.ShortName = 'USD', (1/@CURUSD), (c.Bid/@CURUSD)) Bid

		, iif(a.ShortName = 'USD', (1/@CURUSD), (c.Ask/@CURUSD)) Ask

		, 'USD' as PriceAsset

		, 'MainCurBank' InfoSource

		, 1 Qty

		from QORT_BACK_DB.dbo.CrossRates c

		left outer join QORT_BACK_DB.dbo.Assets a on a.id = c.TradeAsset_ID



		where c.InfoSource = 'CBA'-- and c.OldDate = @todayInt--cast (replace (dateadd(day, -1, cast(cast (GETDATE() as varchar) as date)), '-', '') as INT)

 

 

		SET @CURUSD = (SELECT BID FROM QORT_BACK_DB.dbo.CrossRates WHERE  PriceAsset_ID = 8 AND TradeAsset_ID = 9 and InfoSource = 'ECB')
		...
		, iif(a.ShortName = 'USD', 'USD', a.ShortName) TradeAsset

		, iif(a.ShortName = 'USD', (1), (@CURUSD/c.Bid)) Bid

		, iif(a.ShortName = 'USD', (1), (@CURUSD/c.Ask)) Ask
		...
		left outer join QORT_BACK_DB.dbo.Assets a on a.id = c.PriceAsset_ID

		where c.InfoSource = 'ECB' and a.ShortName in ('DKK', 'HUF', 'IDR', 'ILS', 'KRW', 'MXN', 'MYR', 'NOK', 'PHP', 'RON', 'THB', 'TRY', 'ZAR', 'USD')-- and c.OldDate = @todayInt--cast (replace (dateadd(day, -1, cast(cast (GETDATE() as varchar) as date)), '-
', '') as INT)
	...
	begin catch
		...
		set @Message = 'ERROR: ' + ERROR_MESSAGE(); insert into QORT_ARM_SUPPORT.dbo.uploadLogs(logMessage, errorLevel) values (@message, 1001);
