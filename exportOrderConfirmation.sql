...
/*
	...
	exec QORT_ARM_SUPPORT.dbo.exportOrderConfirmation2 @OrderId = 5648
...
BEGIN
	...
	begin try
		...
		*/
		...
		declare @SheetOutputStart varchar(4) = '1'
		...
		declare @MainQuantity float

		declare @MainSecurityType varchar(128)

		declare @MainPrice float

		declare @MainBaseValue float

		declare @MainPutPlannedDate int
		...
		declare @PlaceOfSettlement varchar(255)
		...
		declare @IsChangeTrade bit = 0

		declare @ChangeRepoDate int = 0

		declare @ChangeRepoAmount float = 0

		declare @RepoNature nvarchar(128) = N'նոր կնքված'
		...
		declare @IsFirm bit = 0

		declare @ClientRepresentative varchar(255)

		declare @AuthorizedRepresentative varchar(255)
...
выбран Counterparty (корт)  - ArmBrok- internal 
			...
			, @IsChangeTrade = iif(ti.Type = 33, 1, 0)
			...
			, @OtherInstructions = replace(isnull(com12.com12, ''), char(13), '')

			, @2OtherInstructions = isnull(com13.com13, '')

			, @3OtherInstructions = isnull(com14.com14, '')

			, @IsFirm = iif(fo.IsFirm = 'y', 1, 0)

			, @ClientRepresentative = isnull('Authorized Representative: ' + nullif(fcr.Name, ''), iif(fo.IsFirm = 'y', '', fo.Name))

			, @AuthorizedRepresentative = isnull(nullif(fcr.Name, ''), '')
		...
		left outer join QORT_BACK_DB.dbo.Firms fcr with (nolock) on fcr.id = ti.Firm_ID
		...
		declare @NewFileName varchar(255) = 'order_' + cast(@OrderID as varchar) + iif(@IsChangeTrade = 1, '_change', '') + iif(@repoStep = 1, '_confo', '') + '_at_' + convert(varchar, getdate(), 112) + '_' + replace(convert(varchar, getdate(), 108), ':', '') +
 '.xlsx'

		declare @NewFileNameRes varchar(255) = 'order_' + cast(@OrderId as varchar) + iif(@IsChangeTrade = 1, '_change', '') + iif(@repoStep = 1, '_confo', '') + '.xlsx'



		if @IsChangeTrade = 1 begin

			select top 1 @MainSecurityType = isnull(AssetSort.SecType, @SecurityType)

				, @MainQuantity = ti0.Qty

				, @MainBaseValue = isnull(a.BaseValue, 0)

				, @MainPrice = ti0.Price

				, @MainPutPlannedDate = ti0.PutPlannedDate

			from QORT_BACK_DB.dbo.TradeInstrs ti with (nolock)

			left outer join QORT_BACK_DB.dbo.TradeInstrLinks til with (nolock) on til.TradeInstr_ID = ti.id

			left outer join QORT_BACK_DB.dbo.TIChangeTrades ct with (nolock) on ct.TradeInstr_ID = ti.id

			left outer join QORT_BACK_DB.dbo.TradeInstrLinks til0 with (nolock) on til0.Trade_ID = ct.Trade_ID

			left outer join QORT_BACK_DB.dbo.TradeInstrs ti0 with (nolock) on ti0.id = til0.TradeInstr_ID

			left outer join QORT_BACK_DB.dbo.Securities sec with (nolock) on sec.id = ti0.Security_ID
		...
		if @IsRepo = 1 and @repoStep = 1 begin
			...
			select top 1 @TradeDate = t1.TradeDate
				...
				, @TradeTime = isnull(pr2.PhaseTime, t1.TradeTime)
				...
				--, @Volume2 = isnull(iif(th.IsAccrued = 'n' and IsBond = 1, th.Volume1 + th.Accruedint, th.Volume1) , iif(t2.IsAccrued = 'n' and IsBond = 1, t2.Volume1 + t2.Accruedint, t2.Volume1))

				, @Volume2 = iif(t1.IsAccrued = 'n' and IsBond = 1, t1.Volume1 + t1.Accruedint, t1.Volume1) * (1 + @RepoTerm * @RepoRate / 100.0 / 365.0)
			...
			left outer join QORT_BACK_DB.dbo.TIChangeTrades tic with (nolock) on tic.TradeInstr_ID = ti.id

			inner join QORT_BACK_DB.dbo.Trades t1 with (nolock) on t1.id = isnull(til.Trade_ID, tic.Trade_ID) and t1.IsRepo2 = 'n'
			...
			outer apply (
				...
				select top 1 tic2.TradeInstr_ID, ti2.RegisterNum, ti2.Date, ti2.Time

				from QORT_BACK_DB.dbo.TIChangeTrades tic2 with (nolock)

				inner join QORT_BACK_DB.dbo.TradeInstrs ti2 with (nolock) on ti2.id = tic2.TradeInstr_ID

				where tic2.Trade_ID = t1.id

					and ((ti2.Date > ti.Date) or (ti2.Date = ti.Date and ti2.Time > ti.Time))

					and ti2.Enabled = 0

				order by ti2.Date, ti2.Time
			...
			outer apply (
				...
				where p.IsCanceled = 'n' and p.PC_Const = 9 and p.Trade_ID = t1.id
					...
					and p.PhaseDate >= ti.Date

					and p.PhaseDate < isnull(n.Date, 33332211)

					/*and p.PhaseDate between ti.Date and isnull(n.Date, 33332211)

					and not (p.PhaseDate = ti.Date and p.PhaseTime < ti.Time and til.TradeInstr_ID is null)

					and not (p.PhaseDate = isnull(n.Date, 0) and p.PhaseTime > isnull(ti.Time, 0))*/

			) com

			outer apply (

				select top 1 p.PhaseDate, p.PhaseTime, t.RepoTrade_ID

				from QORT_BACK_DB.dbo.Trades t with (nolock)

				left outer join QORT_BACK_DB.dbo.Phases p with (nolock) on p.Trade_ID = t.RepoTrade_ID and p.IsCanceled = 'n' and p.PC_Const = 13 --Payment rescheduling
			...
			outer apply (
				...
				select top 1 Volume1, IsAccrued, Accruedint, Corrected_Date, Corrected_Time

				from QORT_BACK_DB.dbo.TradesHist th with (nolock)

				where th.Founder_ID = pr.RepoTrade_ID

					and ((pr.PhaseDate is null) or (pr.PhaseDate > th.Corrected_Date) or (pr.PhaseDate = th.Corrected_Date and pr.PhaseTime > th.Corrected_Time))

				order by Corrected_Date desc, Corrected_Time desc
			...
			if @IsChangeTrade = 1 begin -- CHANGE REPO TRADE CONFIRMATION

				set @ChangeRepoDate = @OrderDate

				set @ChangeRepoAmount = @Volume1



				declare @PrevEndDate int



				select top 1 @EndDate = ti.RepoDate2

					, @PrevEndDate = tih.RepoDate2
				...
				outer apply (
					...
					select top 1 * from (
						...
						select tic2.TradeInstr_ID, ti2.RegisterNum, ti2.Date, ti2.Time

						from QORT_BACK_DB.dbo.TIChangeTrades tic2 with (nolock)

						inner join QORT_BACK_DB.dbo.TradeInstrs ti2 with (nolock) on ti2.id = tic2.TradeInstr_ID

						where tic2.Trade_ID = tic.Trade_ID 
						...
						from QORT_BACK_DB.dbo.TradeInstrLinks tic2 with (nolock)
					...
					) h order by h.Date desc, h.Time desc
				...
				outer apply (
					...
					where p.IsCanceled = 'n' and p.PC_Const = 9 and p.Trade_ID = tic.Trade_ID
						...
						and not (p.PhaseDate = ti.Date and p.PhaseTime < ti.Time)

						and not (p.PhaseDate = isnull(n.Date, 0) and p.PhaseTime > isnull(ti.Time, 0))*/

				) com

				left outer join QORT_BACK_DB.dbo.TradeInstrs tih with (nolock) on tih.id = h.TradeInstr_ID
				...
				if @EndDate > @PrevEndDate set @RepoNature = N'երկարաձգված'

					else if @EndDate < @PrevEndDate set @RepoNature = N'Կրճատված'
...
ndividuals from unfriendly states (as described in President Decree No. 81 of 01.03.2022 (as amended) and Government Order No. 430-r of 05.03.2022 (as amended)).
		...
		if @IsRepo = 1 begin
			...
			if @repoStep = 0 begin
				...
				if @IsChangeTrade = 0 begin
					...
					set @FileNameData = 'order_repo_change_02.xlsx'

					set @FileNameEmpty = 'order_repo_change_02e.xlsx'

					set @Sheet = 'Repo change order'
			...
			set @OtherInstructions = iif(@IsFirm = 1 and len(@OtherInstructions) < 90, 'We hereby confirm that our company is not involved in the import/export of products/services prescribed by the International Financial Sanctions' + char(10), '') 

				+ isnull(@OtherInstructions, '')

				--+ isnull(@PlaceOfSettlement, '')
			...
			set @FileNameEmpty = 'order_buysell_02e1.xlsx'

			set @Sheet = '2 Trade order'

			set @FileDirRes = @FileDirResSPOT

			set @SheetOutputStart = '2'
		...
		alter table ##OrderConfo alter column F5 nvarchar(3000)
...
--select * from ##OrderConfo order by a
		...
		if @IsRepo = 0 begin
			...
			alter table ##MailConfo alter column F3 nvarchar(3000)

			alter table ##MailConfo alter column F6 nvarchar(3000)
		...
		from (values 
			...
			, ('@RepoNature', isnull(@RepoNature, N''))

			, ('@ChangeRepoDate', iif(@IsChangeTrade = 1 and @ChangeRepoDate > 0, QORT_ARM_SUPPORT.dbo.fIntToDateVarchar(@ChangeRepoDate), ''))

			, ('@ChangeRepoAmount', iif(@IsChangeTrade = 1 and @ChangeRepoAmount > 1e-8, QORT_ARM_SUPPORT.dbo.fFloatToMoney2Varchar(@ChangeRepoAmount), ''))
			...
			, ('@PriceAbsolute', iif(@SecurityType <> 'Bonds' and @Price <> 0, QORT_ARM_SUPPORT.dbo.fFloatYieldToVarchar(@Price), '-'))
			...
			, ('@TransactionValue', case when @IsRepo = 0 then iif(@SecurityType <> 'Bonds' and @Price <> 0 and @OrderType <> '@Market', QORT_ARM_SUPPORT.dbo.fFloatToMoney2Varchar(@Price*@Quantity), '-')

					when @IsRepo = 1 then QORT_ARM_SUPPORT.dbo.fFloatToMoney2Varchar(@Quantity * iif(@SecurityType <> 'Bonds', @Price, @Price / 100 * @BaseValue))

				end )			

			, ('@MainTransactionValue', QORT_ARM_SUPPORT.dbo.fFloatToMoney2Varchar(@MainQuantity * iif(@MainSecurityType <> 'Bonds', @MainPrice, @MainPrice / 100 * @MainBaseValue)))			
			...
			, ('@QtyToBuy', iif(@buysell = 'Buy', QORT_ARM_SUPPORT.dbo.fFloatToMoney2Varchar(@Quantity), iif(@tiVolume > 0.01, QORT_ARM_SUPPORT.dbo.fFloatToMoney2Varchar(@tiVolume),'')))

			, ('@QtyToSell', iif(@buysell = 'Sell', iif(@Quantity > 0.01, QORT_ARM_SUPPORT.dbo.fFloatToMoney2Varchar(@Quantity), ''), ''))

			--, ('@Rate', iif(@Price <> 0, QORT_ARM_SUPPORT.dbo.fFloatYieldToVarchar(iif(@buysell = 'Sell', 1 / @Price, @Price)), ''))

			, ('@Rate', iif(@Price <> 0, QORT_ARM_SUPPORT.dbo.fFloatYieldToVarchar(iif(@buysell = 'Sell', @Price, @Price)), ''))
			...
			, ('@MainRepoDeliveryDate', iif(@MainPutPlannedDate > 0, QORT_ARM_SUPPORT.dbo.fIntToDateVarchar(@MainPutPlannedDate), ''))
			...
			, ('@ReceivedDate', QORT_ARM_SUPPORT.dbo.fIntToDateVarchar(@OrderDate))

			, ('@ReceivedTime', iif(@OrderTime > 0, QORT_ARM_SUPPORT.dbo.fIntToTimeVarchar(@OrderTime), ''))

			, ('@ClientRepresentative', isnull(@ClientRepresentative, ''))

			, ('@AuthorizedRepresentative', isnull(@AuthorizedRepresentative, ''))

		) as t(repFrom, repTo)



		update r set r.repTo = replace(repTo, '''', '''''')	from ##repOC r
...
--select F2, F3, F4, F5, F6, F7, F8, F9, F10, F11, F12, F13, F14, F15, F16 from ##OrderConfo order by a
		...
		''SELECT * FROM [' + @Sheet + '$B1:'+char(ascii('B') + @columnsCount-1)+@SheetOutputStart+']'')
...
--select * from ##OrderConfo t
		...
		''SELECT * FROM [' + @Sheet2 + '$B1:'+char(ascii('B') + @columnsCount-1)+@SheetOutputStart+']'')
