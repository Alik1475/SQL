...
-- exec QORT_ARM_SUPPORT_TEST.dbo.upload_REPO_OTC_Edgar @Nom = '404'
...
DATE = GETDATE()
        ...
        DECL
ARE @FileName VARCHAR(128) = '\\192.168.14.22\Exchange\QORT_Files\PRODUCTION\DealsRepo\DealsR.xlsx'
...
24)
    ...
    IF OBJECT_ID('tempdb..##f', 'U') IS NOT NULL DROP TABLE ##f
		...
		IF OBJECT_ID('tempdb..##lt09', 'U') IS NOT NU
LL DROP TABLE ##lt09
		...
		IF OBJECT_ID('tempdb..##lt05
', 'U') IS NOT NULL DROP TABLE ##lt05
		...
		IF OBJECT_I
D('tempdb..##lt01', 'U') IS NOT NULL DROP TABLE ##lt01
  ...
  ''Excel 12.0; Database='+ @FileName + '; HDR=NO;IMEX=1'',
        ...
        FROM OPENROWSET (
            ''Microsoft.ACE.OLEDB.12.0'',
            ''Ex
cel 12.0; Database='+ @FileName + '; HDR=NO;IMEX=1'',
        ...
        FROM OPENROWSET (
            ''Microsoft.ACE.OLEDB.12.0'',
            ''Excel
 12.0; Database='+ @FileName + '; HDR=NO;IMEX=1'',
...
12.0; Database='+ @FileName + '; HDR=NO;IMEX=1'',
        ...
        FROM OPENROWSET (
            ''Microsoft.ACE.OLEDB.12.0'',
            ''Excel 12.0; 
Database='+ @FileName + '; HDR=NO;IMEX=1'',
...
tabase='+ @FileName + '; HDR=NO;IMEX=1'',
        ...
        FROM OPENROWSET (
            ''Microsoft.ACE.OLEDB.12.0'',
            ''Excel 12.0; D
atabase='+ @FileName + '; HDR=NO;IMEX=1'',
...
re [F1] = ''
        ...
        -- Забираем значения из файла E
xcel, в который подгружаются данные из Bloomberg
		CREATE TABLE #t (
...
Insert into QORT_BACK_TDB_UAT.dbo.ImportTrades (
		...
		SELECT 1 as IsProcessed
			...
			, 'CB_RA_FOR_REPO' PutAccount_ExportCode
			...
			, case
					...
					when t.CP = 'HSBC' then (Select FirmShortName from QORT_BACK_DB_UAT..Firms where BOCode = '00885')

			        when t.CP = 'Trinfiko' then (Select FirmShortName from QORT_BACK_DB_UAT..Firms where BOCode = '00804') 

					when t.CP = 'Armswissbank' then (Select FirmShortName from QORT_BACK_DB_UAT..Firms where BOCode = '00035')

					when t.CP = 'Artsakhbank' then (Select FirmShortName from QORT_BACK_DB_UAT..Firms where BOCode = '01146')

					when t.CP = 'Acba' then (Select FirmShortName from QORT_BACK_DB_UAT..Firms where BOCode = '00018')

					when t.CP = 'ArmEconombank' then (Select FirmShortName from QORT_BACK_DB_UAT..Firms where BOCode = '00033')

					when t.CP = 'ABB' then (Select FirmShortName from QORT_BACK_DB_UAT..Firms where BOCode = '00032') --amio bank

					when t.CP = 'Anelik' then (Select FirmShortName from QORT_BACK_DB_UAT..Firms where BOCode = '00347')

					when t.CP = 'Ardshininvestbank' then (Select FirmShortName from QORT_BACK_DB_UAT..Firms where BOCode = '00029')

					when t.CP = 'Ameriabank' then (Select FirmShortName from QORT_BACK_DB_UAT..Firms where BOCode = '00025')

					when t.CP = 'Prometeybank' then (Select FirmShortName from QORT_BACK_DB_UAT..Firms where BOCode = '00059') --evoca bank

					when t.CP = 'Araratbank' then (Select FirmShortName from QORT_BACK_DB_UAT..Firms where BOCode = '00028') 
		...
		FROM #t t
            ...
            WHERE 
                ...
                AND crs.OldDate = CONVERT(INT, SUBSTRING(t.DataFirst,
 7, 4) +  RIGHT('0' + SUBSTRING(t.DataFirst, 4, 2), 2) +  RIGHT('0' + SUBSTRING(t.DataFirst, 1, 2), 2))
            ...
            WHERE 
                crs.TradeAsse
t_ID = aaa.id
                AND crs.OldDate = CONVERT(INT, SUBSTRING(t.DataFirst, 7, 4) +  RIGHT('0' + SUBSTRING(t.DataFirst, 4, 2), 2) +  RIGHT('0' + SUBSTRING(t.DataFirst, 1, 2), 2))
                AND crs.PriceAsset_ID = 17
                AND InfoS
ource = 'CBA'
        ) crsTr
...
nd NOT EXISTS (
		...
		while (@WaitCount > 0 and exists (select top 1 1 from QORT_BACK_TDB_UAT.dbo.ImportTrades q with (nolock) where q.IsProcessed in (1,2)))
...
--*/
			select SUBSTRING(tt.Agrement, CHARI
NDEX('/', tt.Agrement) + 1, LEN(tt.Agrement)) AS RowNum
			,  1 as IsProcessed
			, 2 as ET_Const
			, tt.agrement 
			, 14 PC_Const
			, CONVERT(INT, SUBSTRING(tt.Dataleg1, 7, 4) +  RIGHT('0' + SUBSTRING(tt.Dataleg1, 4, 2), 2) +  RIGHT('0' + SUBSTRING(tt
.Dataleg1, 1, 2), 2)) date
			, (-1) as SystemID
			...
			, CONVERT(INT, SUBSTRING(tt
.Dataleg2, 7, 4) +  RIGHT('0' + SUBSTRING(tt.Dataleg2, 4, 2), 2) +  RIGHT('0' + SUBSTRING(tt.Dataleg2, 1, 2), 2)) DateAfter
			, CONVERT(INT, SUBSTRING(tt.Dataleg1, 7, 4) +  RIGHT('0' + SUBSTRING(tt.Dataleg1, 4, 2), 2) +  RIGHT('0' + SUBSTRING(tt.Dataleg1
, 1, 2), 2)) DateBefore
	...
	--left join QORT_BACK_DB_UAT.dbo.Trades trad with (nolock) on trad.AgreeNum = tt.agrement 
            ...
            FROM QORT_BACK_DB_UAT..Trades trad1 
			left outer join
 QORT_BACK_DB_UAT..Securities sec on sec.id = trad1.Security_ID
			left outer join QORT_BACK_DB_UAT..Assets ass on ass.id = sec.Asset_ID
			WHERE 
                trad1.AgreeNum = LEFT(tt.agrement, CHARINDEX('/', tt.agrement) - 1) collate Cyrillic_General
_CS_AS
		...
		--an
d left(Agrement,3) = @Nom --+ '-24R/1'
		--and right(tt.DataFirst,7) = '09/2024' 
		--and LEFT(tt.agrement,3) = '518'
		 and NOT EXISTS (
				select TOP 1 *
				from QORT_BACK_DB_UAT.dbo.Phases a
				where (a.BackID + '_' + cast(isnull(trad.id,'') as varc
har(16)) = tt.Agrement or a.BackID = tt.Agrement)
...
--*/
		...
		where RowNum = @n
			...
			s
et @WaitCount = 1200 -------------------- задержка, не продолжаем пока не подгрузили фазы по пролонгации по активам----------------------

		while (@WaitCount > 0 and exists (select top 1 1 from QORT_BACK_TDB_UAT.dbo.Phases q with (nolock) where q.IsProcessed in (1,2)))
...
, 1, 2), 2)) DateBefore
			...
			from #t tt
			--left join QORT_BA
CK_DB_UAT.dbo.Trades trad with (nolock) on trad.AgreeNum = tt.agrement 
						        OUTER APPLY (
            SELECT top 1  ass.ShortName, trad1.id, trad1.SubAcc_ID
            FROM QORT_BACK_DB_UAT..Trades trad1 
			left outer join QORT_BACK_DB_UAT..Se
curities sec on sec.id = trad1.Security_ID
			left outer join QORT_BACK_DB_UAT..Assets ass on ass.id = sec.Asset_ID
			WHERE 
...
/*
Insert
 into QORT_BACK_TDB_UAT.dbo.phases (
			IsProcessed
			...
			where RowNum = @n
				...
				s
et @WaitCount = 1200 -------------------- задержка, не продолжаем пока не подгрузили----------------------
...
--*/
			...
			, (-1) as SystemID
			, tt.agrement +'_' + CAST(is
null(trad.id,'') as varchar(16)) as BackID
			, null as InfoSource
			, trad.id Trade_SID
			, round(tt1.VolPercent, 2) as QtyBefore
			, Sub.SubAccCode as SubAcc_Code
			--, CONVERT(INT, SUBSTRING(tt.Dataleg2, 7, 4) +  RIGHT('0' + SUBSTRING(tt.Dataleg2, 
4, 2), 2) +  RIGHT('0' + SUBSTRING(tt.Dataleg2, 1, 2), 2)) DateAfter
...
(tt.Dataleg2, 7, 4) +  RIGHT('0' + SUBSTRING(tt.Dataleg2, 4, 2), 2) +  RIGHT('0' + SUBSTRING(tt.Dataleg2, 1, 2), 2)) NewRepoRateDate
			,  (-1) QtyAfter
			from #t tt
			--left join QORT_BACK_DB_UAT.dbo.Trades trad with (nolock) on trad.AgreeNum = tt.agre
ment 
			
 OUTER APPLY (
            SELECT top 1  ass.ShortName, trad1.id, trad1.SubAcc_ID
            FROM QORT_BACK_DB_UAT..Trades trad1 
			left outer join QORT_BACK_DB_UAT..Securities sec on sec.id = trad1.Security_ID
			left outer join QORT_BACK_DB_
UAT..Assets ass on ass.id = sec.Asset_ID
			WHERE 
                trad1.AgreeNum = LEFT(tt.agrement, CHARINDEX('/', tt.agrement) - 1) collate Cyrillic_General_CS_AS
                  AND Trad1.VT_Const NOT IN (12, 10) -- сделка не расторгнута
...
rst,7) = '09/2024' 
		--and LEFT(tt.agrement,3) = '518'
			 and NOT EXISTS (
				select TOP 1 *
				from QORT_BACK_DB_UAT.dbo.Phases a
				where (a.BackID + '_' + cast(isnull(trad.id,'') as varchar(16)) = tt.Agrement or a.BackID = tt.Agrement)
				and a.E
nabled = 0 and a.IsCanceled = 'n' 
				and a.PC_Const = 5
			)
		...
		, 2 as ET_Const
		  ...
		  and PayAccount_ID = 175 -- CB_RA_FOR_REPO
		  	 and NOT EXISTS (
				...
				from QORT_BACK_DB_UAT.dbo.Phases a
				wher
e (a.BackID = trad.AgreeNum collate Cyrillic_General_CI_AS or a.BackID = trad.AgreeNum +'_' + cast(trad.id as varchar(16)) collate Cyrillic_General_CI_AS)
				and a.Enabled = 0 and a.IsCanceled = 'n' 
				and a.PC_Const = 4
			)
			--/*
			Insert into QOR
T_BACK_TDB_UAT.dbo.phases (
			IsProcessed
...
CH
        ...
        INSERT INTO QORT_ARM_SUPPORT_TEST.dbo.uploadLogs(logMes
sage, errorLevel) VALUES (@Message, 1001);
